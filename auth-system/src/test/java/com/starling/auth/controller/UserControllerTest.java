...
class UserControllerTest {
    ...
    void authMe_ShouldReturnMeResponse_WhenAuthenticated() throws Exception {
        Identity identity = Identity.builder()
                .providerId("http://localhost:8180/realms/starling")
                .externalSubject("sub-123")
                .email("user@starling.invalid")
        ...
        mockMvc.perform(get("/auth/me").with(authentication(auth)))
                ...
                .andExpect(jsonPath("$.providerId").value("http://localhost:8180/realms/starling"))
                .andExpect(jsonPath("$.externalSubject").value("sub-123"))
                .andExpect(jsonPath("$.email").value("user@starling.invalid"))
