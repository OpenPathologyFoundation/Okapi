...
public class KeycloakIntegrationTest {
        ...
        static void dynamicProperties(DynamicPropertyRegistry registry) {
                if (KEYCLOAK_AVAILABLE) {
                        log.info("Configuring Keycloak OIDC issuer to: {}/realms/starling", KEYCLOAK_URL);
                        registry.add("spring.security.oauth2.client.provider.okta.issuer-uri",
                                        () -> KEYCLOAK_URL + "/realms/starling");
                } else {
    ...
    @Autowired
    private com.starling.auth.repository.IdentityRepository identityRepository;
