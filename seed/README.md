...
## Keycloak seeding (users + groups)

To provision Keycloak users/groups from `seed/identities/xenonym-identities.v1.json`, use:
...
Notes:
- The script runs `kcadm.sh` **inside** the Keycloak container, but parses `xenonym-identities.v1.json` on the **host** using `python3`.
...
2. Provision users/groups in Keycloak (this makes sure subjects exist):
...
Notes:
- The endpoint reads `seed/identities/xenonym-identities.v1.json` and upserts by `(provider_id, external_subject)`.
